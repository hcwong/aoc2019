...
typedef std::map<long, std::map<long, int>> VISITED;
typedef std::map<long, int> INNER_MAP;
...
bool isTileBlack(state *s, long x, long y);
void updateDirection(state *s, int value);
void updatePaint(state *s, int newColor);
std::vector<std::vector<char>> generateMatrix(state *s);
...
int main() {
    ...
    INNER_MAP initial;
    initial[0] = 1;
    s.visited.insert(std::pair<long, INNER_MAP>(0, initial));
    ...
    auto matrix = generateMatrix(&s);
    for (std::vector<std::vector<char>>::iterator it = matrix.begin(); it != matrix.end(); it++) {
        for (std::vector<char>::iterator inner_it = it->begin(); inner_it != it->end(); inner_it++) {
            std::cout << *inner_it;
        }
        std::cout << '\n';
    }
...
int processOpcode(std::vector<long> &nums, state *s) {
    ...
    switch(opcode) {
        ...
        case 2:
            ...
            return 0;
        case 3:
            input = isTileBlack(s, s->x, s->y) ? 0 : 1;
...
bool isTileBlack(state *s, long x, long y) {
    if (s->visited.count(x) == 0) {
        return true;
    }

    // If not, check the y
    if (s->visited.find(x)->second.count(y) == 0) {
        return true;
    }

    // Else update
    int tile = s->visited.find(x)->second.find(y)->second;
    return tile == 0;
}
...
std::vector<std::vector<char>> generateMatrix(state *s) {
    long minX = s->visited.begin()->first;
    long maxX = s->visited.begin()->first;

    // assume x is never empty
    long minY = s->visited.begin()->second.begin()->first;
    long maxY = s->visited.begin()->second.begin()->first;

    for (VISITED::iterator it = s->visited.begin(); it != s->visited.end(); it++) {
        if (it->first > maxX) {
            maxX = it->first;
        }

        for (INNER_MAP::iterator inner_it = it->second.begin(); inner_it != it->second.end(); inner_it++) {
            int current_value = inner_it->first;
            if (current_value < minY) {
                minY = current_value;
            }

            if (current_value > maxY) {
                maxY = current_value;
            }
        }
    }
    ...
    for (long i = 0; i < lenX; i++) {
        ...
        for (long j = 0; j < lenY; j++) {
            long coordX = i + minX;
            long coordY = j + minY;
            if (isTileBlack(s, coordX, coordY)) {
                inner.push_back(' ');
            } else {
                inner.push_back('#');
            }
